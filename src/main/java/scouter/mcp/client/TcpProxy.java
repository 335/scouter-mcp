// Ported from scouter.webapp v2.20.0 (Apache License 2.0) — source: scouterx.webapp.framework.client.net.TcpProxy
/*
...
public class TcpProxy implements AutoCloseable {
    ...
    protected TcpProxy(int serverId) {
        this.server = ServerRegistry.get(serverId);
    ...
    public static synchronized TcpProxy getTcpProxy(final Server server) {
        Server _server = ServerRegistry.getServerIfNullDefault(server);
        if (_server == null || _server.isOpen() == false) {
            throw McpError.of(McpError.Code.SCOUTER_CONNECT_FAILED, "server is not exist or before initializing! - "
    ...
    public static synchronized TcpProxy getTcpProxy(int serverId) {
        return getTcpProxy(ServerRegistry.get(serverId));
    }
    ...
    public synchronized void process(String cmd, Object param, INetReader recv) {
        open();
        if (tcp.connected() == false) {
            throw McpError.of(McpError.Code.SCOUTER_CONNECT_FAILED, "[TcpProxy.process] client socket closed.");
        }
