    ...
    public static final String SEARCH_XLOG = """
        {
          ...
          "properties": {
            ...
            "service": {"type": "string", "description": "Filter: service (request URL) name. Sloppy input is fine — 'orderDetail', 'GET /api/order/order-detail', 'order-detail POST', or a pasted 'name<POST>' are all normalized (HTTP method extracted, longest token used server-side). Matching is case-sensitive substring; if nothing matches, the result returns serviceCandidates with real service names from the same window — retry with one of those. Advanced: '*' patterns are used as-is. NOTE: a service filter scans raw XLogs per instance and is slow on production (may time out with a partial result) — to trace one service, first get its EXACT name and busiest instance cheaply from get_summary (category=service), then search here with that exact name + a single objHash + a narrow window."},
    ...
    public static final String SERVICE_SUMMARY = """
        {
          ...
          "properties": {
            ...
            "to": {"type": "string", "description": "End time (e.g. now)"},
            "objNameLike": {"type": "string", "description": "Fuzzy target: app-name fragment (e.g. 'shop-order-api'). Case-insensitive; aggregates over ALL matching instances, including pods replaced by a deploy during the queried window. PREFER THIS over objHash. If unsure of the app or the fragment could match several different apps, confirm with the user before a broad fan-out — each instance is a separate slow scan."},
            "objHash": {"type": "integer", "description": "Filter: a specific object hash (advanced; prefer objNameLike)"},
            "service": {"type": "string", "description": "Filter: service (request URL) name. Sloppy input is normalized like search_xlog; on zero matches, serviceCandidates lists real names. NOTE: filtering by service scans raw XLogs per instance and is slow on production — to target one service, first get its EXACT name from get_summary (category=service), then narrow with a single objHash."},
